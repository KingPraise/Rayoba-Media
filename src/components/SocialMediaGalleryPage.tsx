...
import { SOCIAL_MEDIA_IMAGES, createProjectFromSocialImage } from '../data';
...
export default function SocialMediaGalleryPage({ onNavigateToPage }: Props) {
    ...
    <div className="w-full relative min-h-screen pt-28 pb-16 flex flex-col justify-between">
      <section className="max-w-7xl mx-auto px-6 md:px-12 w-full mt-8 md:mt-16 text-center">
        ...
        <h1 className="font-display text-4xl md:text-6xl font-black text-gray-950 dark:text-gilded-ivory tracking-tight mb-6 leading-tight">
          ...
          <span className="text-royal-purple italic font-light">Digital Campaigns</span>
        </h1>
        <p className="font-sans text-sm md:text-base text-gray-600 dark:text-gray-400 max-w-2xl mx-auto mb-16 leading-relaxed">
          The full collection of our high-impact social media assets. Engineered for absolute aesthetic dominance and optimal audience conversion.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 text-left">
          {SOCIAL_MEDIA_IMAGES.map((img, idx) => (
            <motion.div
              key={img.id}
              initial={{ opacity: 0, y: 25 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5 }}
              className="group relative rounded-[28px] border border-royal-gold/15 bg-white/60 dark:bg-zinc-950/20 shadow-2xl overflow-hidden flex flex-col justify-between aspect-square cursor-pointer"
              onClick={() => setSelectedProject(createProjectFromSocialImage(img, idx))}
            >
